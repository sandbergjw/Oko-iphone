...
def clean_flyer_product_name(name):
    """Rens OCR-navnet og fjern logo-/kampagnetekst uden at gætte på varen."""
    name = clean_flyer_ocr_text(name)

    # Sammenfald af logo-OCR: "ØGO ØGO", "ØGO SØGO" osv.
    name = re.sub(r"(?i)\bØGO\s+ØGO\b", "ØGO", name)
    name = re.sub(r"(?i)\bØGO\s+SØGO\b", "ØGO", name)

    # Fjern ord som ikke er en del af selve varen.
    name = re.sub(r"(?i)\b(?:aktuel|tilbudsavis|spot|spotvarer)\b", "", name)
    name = re.sub(r"(?i)\bpr\.?\s+(?:kunde|dag|stk|pose|bakke|kg|liter)\b.*$", "", name)
    name = re.sub(r"\s+", " ", name).strip(" -–·,.;:")

    # Behold gerne ØGO i teknisk råtekst, men i den synlige varetekst
    # er det mere læsbart at vise selve produktet.
    visible = re.sub(r"(?i)^\s*(?:ØGO|ØKO)\s*", "", name).strip()
    visible = re.sub(r"(?i)^\s*økologisk(?:e)?\s*", "", visible).strip()

    # "eller tomater" er typisk OCR, hvor første produktnavn er faldet væk.
    # Vis kun den sikre del fremfor den mærkelige frase.
    visible = re.sub(r"(?i)^eller\s+", "", visible).strip()

    # Tom/ren logo-linje er ikke et produkt.
    if normalize(visible) in {"", "øgo", "øko", "søgo", "ogo"}:
        return ""
...
def flyer_name_quality(name):
    """Konservativ kvalitetskontrol før en OCR-vare vises i den normale liste."""
    n = normalize(name)
    if not n:
        return 0

    bad_phrases = [
        "pr kunde pr dag", "spotvarer fås", "annonce",
        "gode vaner", "først til mølle",
    ]
    ...
    # Rene logo-/OCR-rester.
    if len(words) == 1 and words[0] in {"øgo", "øko", "ogo", "søgo"}:
        return 0
...
st.caption("Øko-robot v2.1.2 · strammere OCR-navne")
